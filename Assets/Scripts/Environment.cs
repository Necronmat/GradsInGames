...
public class Environment : MonoBehaviour
{
    ...
    public EnvironmentTile End { get; private set; }
    ...
    private void Generate()
    {
        ...
        Vector3 position = new Vector3(-10.0f, 0.0f, 10.0f); //Origenal Vector3( -(halfWidth * TileSize), 0.0f, -(halfHeight * TileSize) )
        ...
        //The end position must be at least half the map size away from the start position
        Vector2Int endPosition = new Vector2Int();
        do
        {
            endPosition.x = Random.Range(2, Size.x - 2);
            endPosition.y = Random.Range(2, Size.y - 2);
        } while (Mathf.Abs(endPosition.x - startPosition.x) <= Size.x / 3 || Mathf.Abs(endPosition.y - startPosition.y) <= Size.y / 3);
        ...
        for ( int x = 0; x < Size.x; ++x)
        {
            ...
            for ( int y = 0; y < Size.y; ++y)
            {
                ...
                //If there are multiple empty tiles use tiles[Random.Range(0, tiles.Count)]
                EnvironmentTile prefab = tiles[0];
                EnvironmentTile tile;               

                //Decides whether the current tile is a special case (Start, End, Outer Wall)
                //or if a random tile can be made
                if (x == startPosition.x && y == startPosition.y)
                {
                    tile = Instantiate(AccessibleTiles[0], position, Quaternion.identity, transform);
                    tile.IsAccessible = true;
                    start = true;
                }
                else if(x == endPosition.x && y == endPosition.y)
                {
                    tile = Instantiate(AccessibleTiles[1], position, Quaternion.identity, transform);
                    tile.IsAccessible = true;
                    end = true;
                }
                else if(x == 1 && y == 0)
                ...
                else if(x == 0 || y == 0 || x == Size.x - 1 || y == Size.y - 1)
                {
                    tile = Instantiate(InaccessibleTiles[0], position, Quaternion.identity, transform);
            ...
            position.x += TileSize;
            position.z = 10;//Origenal = -(halfHeight * TileSize)
        }
    }
